...
| クライアント側画像圧縮 | Canvas APIで800pxリサイズ + JPEG品質70%に圧縮してからアップ |
...
| Lambda SnapStart | 初期化済みスナップショットからの高速起動 |
...
| コンピュート | AWS Lambda (Python 3.12, **ARM64 Graviton2, 10GB, Provisioned Concurrency, SnapStart**) | 画像処理の各ステップ（コールドスタート排除・CPU最大化） |
